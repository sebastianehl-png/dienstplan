...
export type GridData = {
  ...
  cells: Record<string, GridCell>; // `${rowKey}|${day}|${slot}`
...
export default function WeekGrid({ data }: { data: GridData }) {
  ...
  const [textMode, setTextMode] = useState<Set<string>>(new Set()); // `${rowKey}|${day}|${slot}`
  ...
  function setCell(rowKey: string, day: number, slot: number, value: string) {
    if (value === "__TEXT__") {
      setTextMode((cur) => new Set(cur).add(`${rowKey}|${day}|${slot}`));
      return;
    }
    const userId = value === "" ? null : value;
    start(async () => {
      await updateWeekCell(data.weekStart, rowKey, day, slot, userId, null);
      router.refresh();
    });
  }
  function setCellText(rowKey: string, day: number, slot: number, text: string) {
    const clean = text.trim();
    if (!clean) {
      setTextMode((cur) => {
        const next = new Set(cur);
        next.delete(`${rowKey}|${day}|${slot}`);
        return next;
      });
    }
    start(async () => {
      await updateWeekCell(data.weekStart, rowKey, day, slot, null, clean || null);
      router.refresh();
    });
  }
    ...
    <div className="space-y-4">
      ...
      {report && (report.gaps.length > 0 || report.doubleBookings.length > 0 || report.defaultsReplaced.length > 0) && (
        <div className="space-y-2 rounded-xl border border-amber-200 bg-amber-50/50 p-4 text-sm">
          ...
          {report.gaps.map((g, i) => (
            <div key={`g${i}`} className="text-red-700">⚠ Unbesetzt: {rowLabel(g.rowKey)} / {DAY_NAMES[g.day]} — {g.reason}</div>
          ))}
          {report.defaultsReplaced.map((d, i) => (
            <div key={`d${i}`} className="text-amber-800">↷ Standard ersetzt: {rowLabel(d.rowKey)} / {DAY_NAMES[d.day]} — {d.defaultName} ({d.reason})</div>
          ))}
          {report.doubleBookings.map((b, i) => (
            <div key={`b${i}`} className="text-amber-800">⚠ Doppelbelegung: {b.userName} — {rowLabel(b.rowKey)} / {DAY_NAMES[b.day]}</div>
          ))}
        </div>
      )}
        ...
        <div className="overflow-x-auto rounded-xl border border-zinc-200 bg-white">
          <table className="w-full border-collapse text-xs">
            ...
            <tbody>
              {WEEK_ROWS.map((row) => (
                <tr key={row.key}>
                  <td className="max-w-44 border-t border-zinc-100 px-3 py-1.5 font-medium text-zinc-800">
                    {row.label}
                    {(row.slots ?? 1) > 1 && <span className="ml-1 font-normal text-zinc-400">(bis {row.slots})</span>}
                  </td>
                  {[0, 1, 2, 3, 4].map((day) => {
                    if (row.grayDays.includes(day)) return <td key={day} className="border-t border-l border-zinc-100 bg-zinc-200/70" />;

                    if (row.autoAbsences) {
                      const cell = data.cells[`${row.key}|${day}|0`] ?? { userId: null, text: null };
                      return (
                        <td key={day} className="border-t border-l border-zinc-100 px-1 py-1">
                          <input
                            defaultValue={cell.text ?? ""}
                            onBlur={(e) => e.target.value !== (cell.text ?? "") && setCellText(row.key, day, 0, e.target.value)}
                            placeholder="–"
                    ...
                    const options = data.eligible[`${row.key}|${day}`] ?? [];
                    // in Geschwister-Plätzen gewählte Personen ausblenden
                    const chosen = new Set(
                      Array.from({ length: slots }, (_, s) => data.cells[`${row.key}|${day}|${s}`]?.userId).filter(Boolean) as string[]
                    );

                    return (
                      <td key={day} className="border-t border-l border-zinc-100 px-1 py-1 align-top">
                        <div className="space-y-0.5">
                          {Array.from({ length: slots }, (_, slot) => {
                            const key = `${row.key}|${day}|${slot}`;
                            ...
                            if (inTextMode) {
                              return (
                                <input
                                  ...
                                  defaultValue={cell.text ?? ""}
                                  onBlur={(e) => e.target.value.trim() !== (cell.text ?? "") && setCellText(row.key, day, slot, e.target.value)}
                                  onKeyDown={(e) => e.key === "Enter" && (e.target as HTMLInputElement).blur()}
                                  placeholder="Freitext… (leer = zurück)"
                                  className="w-full rounded border border-transparent bg-amber-50/60 px-1 py-0.5 text-xs hover:border-zinc-300 focus:border-blue-500 focus:outline-none"
                                />
                              );
                            }
                            const invalid = cell.userId != null && !options.includes(cell.userId);
                            const slotOptions = options.filter((id) => !chosen.has(id) || id === cell.userId);
                            // Leere Zusatz-Plätze kompakt halten: nur anzeigen, wenn belegt oder erster leerer
                            const prevFilled =
                              slot === 0 ||
                              data.cells[`${row.key}|${day}|${slot - 1}`]?.userId != null ||
                              data.cells[`${row.key}|${day}|${slot - 1}`]?.text != null ||
                              textMode.has(`${row.key}|${day}|${slot - 1}`);
                            if (!cell.userId && !prevFilled) return null;
                            return (
                              <select
                                key={slot}
                                value={cell.userId ?? ""}
                                onChange={(e) => setCell(row.key, day, slot, e.target.value)}
                                title={invalid ? "Person ist abwesend, gesperrt oder ohne Fähigkeit" : undefined}
                                className={`w-full rounded border border-transparent bg-transparent px-1 py-0.5 text-xs hover:border-zinc-300 focus:border-blue-500 focus:outline-none ${invalid ? "bg-red-50 text-red-700" : ""}`}
                              >
                                <option value="">–</option>
      ...
      {data.status !== "NONE" && (
        <p className="text-xs text-zinc-400">
          Dropdowns zeigen nur Personen mit gültiger Fähigkeit, die anwesend und nicht gesperrt sind (rot = Bedingung verletzt).
          Zeilen mit „(bis N)" können mehrfach besetzt werden – weitere Plätze erscheinen, sobald der vorherige belegt ist.
          Die Rufbereitschaft kommt aus dem Jahres-Dienstplan (Fr = Wochenend-Rufdienst).
        </p>
      )}
    </div>
  );
